...
public class CampfireModifierRack extends BlockWithEntity implements BlockEntityProvider {
        ...
        if(world.getBlockEntity(pos) instanceof CampfireModifierRackEntity campfireModifierRackEntity) {
            ItemStack itemStack = player.getStackInHand(hand);
                if (world instanceof ServerWorld serverWorld) {
                    if (hasRecipe(serverWorld, stack)) {
    ...
    private boolean hasRecipe(ServerWorld world, ItemStack stack){
        Optional<RecipeEntry<CampfireSmokingRecipe>> recipe = world.getRecipeManager().getFirstMatch(ModRecipes.CAMPFIRE_SMOKING_TYPE,new SingleStackRecipeInput(stack), world);
        MaximumGohan.LOGGER.info("{}",recipe.toString());
        return true;
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world instanceof ServerWorld serverWorld) {
                ...
                return validateTicker(type, ModBlockEntities.RACK_BE, (worldx, pos, statex, blockEntity) -> CampfireModifierRackEntity.serverTick(serverWorld, pos, statex, blockEntity, matchGetter));
        } else {
